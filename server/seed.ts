...
async function seed() {
  ...
  const existingMfrs = await sql`SELECT name FROM manufacturers WHERE name IN ('Zinchar', 'Nutrimed')`;
  const existingMfrNames = existingMfrs.map((r) => r.name);

  if (!existingMfrNames.includes("Zinchar")) {
    await sql`INSERT INTO manufacturers (name, standard_lead_time_days, max_lead_time_days) VALUES ('Zinchar', 40, 60)`;
  ...
  if (!existingMfrNames.includes("Nutrimed")) {
    await sql`INSERT INTO manufacturers (name, standard_lead_time_days, max_lead_time_days) VALUES ('Nutrimed', 40, 60)`;
    console.log("  Inserted manufacturer: Nutrimed");
  } else {
    console.log("  Manufacturer Nutrimed already exists, skipping.");
  }

  // Fetch manufacturer IDs for FK references
  const mfrRows = await sql`SELECT id, name FROM manufacturers WHERE name IN ('Zinchar', 'Nutrimed')`;
  ...
  const products: ProductDef[] = [
    // ── Horse Mixes (THH, HORSE_MIX, no units_per_case, location=THH, default mfr=Zinchar) ──
    ...
    // ── Pet Formulas (THH, PET_FORMULA, units_per_case=6, location=THH, default mfr=Zinchar) ──
    ...
    // ── Chews (THH, CHEW, units_per_case=12, location=THH, default mfr=Zinchar) ──
    ...
    // ── Spray (THH, SPRAY, units_per_case=6, default mfr=Zinchar) ──
    ...
    // ── Shampoo (THH, SHAMPOO, units_per_case=6, default mfr=Zinchar) ──
    ...
    // ── Gravy (THH, GRAVY, units_per_case=6, default mfr=Zinchar) ──
    ...
    // ── Nutriphase (NP brand, all mfr=Nutrimed, location=88) ──
  ...
  const existingProducts = await sql`SELECT sku_code FROM products`;
  ...
  for (const p of products) {
    if (existingSkus.has(p.sku_code)) {
      console.log(`  Skipping ${p.sku_code} (already exists)`);
      skippedCount++;
      continue;
    }

    await sql`
      INSERT INTO products (sku_code, product_name, brand, category, pack_size_g, units_per_case, manufacturer_id, primary_stock_location, xero_item_code, is_active)
      VALUES (${p.sku_code}, ${p.product_name}, ${p.brand}, ${p.category}, ${p.pack_size_g}, ${p.units_per_case}, ${p.manufacturer_id}, ${p.primary_stock_location}, ${p.sku_code}, true)
    `;
  ...
  const existingApMappings = await sql`SELECT ap_product_code FROM ap_brand_mappings`;
  const existingApCodes = new Set(existingApMappings.map((r) => r.ap_product_code));

  for (const m of apMappings) {
    if (existingApCodes.has(m.ap_product_code)) {
      console.log(`  Skipping AP mapping ${m.ap_product_code} (already exists)`);
      continue;
    }

    await sql`
      INSERT INTO ap_brand_mappings (ap_product_code, thh_sku_code, ap_product_name)
      VALUES (${m.ap_product_code}, ${m.thh_sku_code}, ${m.ap_product_name})
    `;
  ...
  const existingSetting = await sql`SELECT id FROM system_settings WHERE key = 'pnp_dc_codes'`;

  if (existingSetting.length > 0) {
    await sql`UPDATE system_settings SET value = ${JSON.stringify(pnpDcCodes)}, updated_at = NOW() WHERE key = 'pnp_dc_codes'`;
    console.log("  Updated existing pnp_dc_codes setting.");
  } else {
    await sql`INSERT INTO system_settings (key, value) VALUES ('pnp_dc_codes', ${JSON.stringify(pnpDcCodes)})`;
